...
parser.add_argument('--db_cluster_name', dest='db_cluster_name', help='Enter Aurora DB Cluster name')
parser.add_argument('--kms_key_id', dest='kms_key_id', help='Enter KMS key ID')
parser.add_argument('--region', dest='region', help='Enter AWS Region')
parser.add_argument('--access_key', dest='access_key', help='Enter AWS Access Key')
parser.add_argument('--secret_key', dest='secret_key', help='Enter AWS Secrete Key')
...
def encrypt_rds():
    ...
    db_cluster_name = results.db_cluster_name
    customer_master_key = results.kms_key_id
    ...
    client = boto3.client('rds', region_name=region, aws_access_key_id=results.access_key, aws_secret_access_key=results.secret_key)
    ...
    if('errorMessage' in response):
        ...
        for member in db_cluster_members:
            ...
            print("  Adding {0} instances to RDS cluster completed...".format(instance_response['DBInstance']['DBInstanceIdentifier']))
...
credentials = boto3.Session(aws_access_key_id=results.access_key, aws_secret_access_key=results.secret_key, region_name=results.region).get_credentials()
encrypt_rds()